(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('@valeera/timeline'), require('@valeera/eventfirer'), require('@valeera/x'), require('@valeera/mathx'), require('@valeera/tree')) :
	typeof define === 'function' && define.amd ? define(['exports', '@valeera/timeline', '@valeera/eventfirer', '@valeera/x', '@valeera/mathx', '@valeera/tree'], factory) :
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.Nova = {}, global.Timeline, global.EventFirer, global.x$1, global.mathx, global.tree));
})(this, (function (exports, Timeline, EventFirer, x$1, mathx, tree) { 'use strict';
	...
	// 既可以是2d几何体也可以是3D几何体
	class Geometry extends x$1.Component {
	    /**
	     * 顶点数量
	     */
	    count;
	...
	var createBox3 = (options = {}) => {
	    ...
	    function buildPlane(u, v, w, udir, vdir, width, height, depth, gridX, gridY) {
	        ...
	        let vertexCounter = 0;
	        const vector = new mathx.Vector3();
	...
	const DEFAULT_SPHERE_OPTIONS$1 = {
	    ...
	    openEnded: false,
	    thetaStart: 0,
	    thetaLength: mathx.Constants.DEG_360_RAD,
	    cullMode: "back"
	};
	var createCylinder3 = (options = {}) => {
	    ...
	    function generateTorso() {
	        const normal = new mathx.Vector3();
	        ...
	        for (let y = 0; y <= heightSegments; y++) {
	            ...
	            for (let x = 0; x <= radialSegments; x++) {
	                ...
	                normal[1] = slope;
	                normal[2] = cosTheta;
	                mathx.Vector3.normalize(normal, normal);
	...
	var createTriangle3 = (t = mathx.Triangle3.create(), options = DEFAULT_OPTIONS, topology = "triangle-list", cullMode = "none") => {
	    ...
	    if (options.combine) {
	        ...
	        result.set(t.c, stride + stride);
	        if (options.hasNormal) {
	            let normal = mathx.Triangle3.normal(t);
	            result.set(normal, 3);
	            result.set(normal, stride + 3);
	            result.set(normal, stride + stride + 3);
	            pickers.push({
	...
	var createTriangle2 = (t = mathx.Triangle2.create(), options = DEFAULT_OPTIONS, topology = "triangle-list", cullMode = "none") => {
	...
	class Material extends x$1.Component {
	    tags = [{
	            label: MATERIAL,
	            unique: true
	        }];
...
@fragment fn main(@location(0) normal : vec4<f32>) -> @location(0) vec4<f32> {
	...
	class Sampler extends x$1.Component {
	    data = {
	        minFilter: 'linear',
	        magFilter: 'linear',
	    };
	...
	class Matrix3Component extends x$1.Component {
	    constructor(name, data = mathx.Matrix3.create(), tags = []) {
	...
	const updateModelMatrixComponent$1 = (mesh) => {
	    ...
	    if (p3?.dirty || r3?.dirty || s3?.dirty || a3?.dirty) {
	        mathx.Matrix3.fromArray(p3?.data || mathx.Matrix3.UNIT_MATRIX3, m3.data);
	        if (r3) {
	            mathx.Matrix3.multiplyRotationMatrix(m3.data, r3.data, m3.data);
	        }
	        if (s3) {
	            mathx.Matrix3.multiplyScaleMatrix(m3.data, s3.data, m3.data);
	        }
	        if (a3) {
	            mathx.Matrix3.multiplyTranslateMatrix(m3.data, a3.data, m3.data);
	        }
	        if (p3) {
	            p3.dirty = false;
	        }
	    ...
	    if (mesh.parent) {
	        let parentWorldMatrix = mesh.parent.worldMatrix?.data ?? mathx.Matrix3.UNIT_MATRIX3;
	        mathx.Matrix3.multiply(parentWorldMatrix, m3.data, worldMatrix.data);
	    }
	    else {
	        mathx.Matrix3.fromArray(m3.data, worldMatrix.data);
	    }
	    return m3;
	};

	class Anchor2 extends Matrix3Component {
	    vec2 = new mathx.Vector2();
	    constructor(vec = mathx.Vector2.VECTOR2_ZERO) {
	        super(ANCHOR_2D, mathx.Matrix3.create(), [{
	                label: ANCHOR_2D,
	                unique: true
	            }]);
	        mathx.Vector2.fromArray(vec, 0, this.vec2);
	        this.update();
	    }
	    get x() {
	        return this.vec2[0];
	...
	class APosition2 extends Matrix3Component {
	    constructor(data = mathx.Matrix3.create()) {
	...
	class EuclidPosition2 extends APosition2 {
	    ...
	    constructor(vec2 = new Float32Array(2)) {
	        super();
	        mathx.Vector2.fromArray(vec2, 0, this.vec2);
	        this.update();
	    }
	    get x() {
	        return this.vec2[0];
	    ...
	    update() {
	        mathx.Matrix3.fromTranslation(this.vec2, this.data);
	        this.dirty = true;
	        return this;
	    }
	}
	...
	class AngleRotation2 extends ARotation2 {
	    ...
	    update() {
	        mathx.Matrix3.fromRotation(this.#angle, this.data);
	        this.dirty = true;
	        return this;
	    }
	}
	...
	const orthogonal = (left, right, bottom, top, out = new mathx.Matrix3()) => {
	...
	class Vector2Scale2 extends AScale2 {
	    ...
	    update() {
	        mathx.Matrix3.fromScaling(this.vec2, this.data);
	        return this;
	    }
	}

	class Matrix4Component extends x$1.Component {
	    constructor(name, data = mathx.Matrix4.create(), tags = []) {
	        super(name, data, tags);
	        this.dirty = true;
	    }
	}
	const updateModelMatrixComponent = (mesh) => {
	    ...
	    let worldMatrix = mesh.worldMatrix;
	    if (p3?.dirty || r3?.dirty || s3?.dirty || a3?.dirty) {
	        mathx.Matrix4.fromArray(p3?.data || mathx.Matrix4.UNIT_MATRIX4, m3.data);
	        if (r3) {
	            mathx.Matrix4.multiply(m3.data, r3.data, m3.data);
	        }
	        if (s3) {
	            mathx.Matrix4.multiplyScaleMatrix(m3.data, s3.data, m3.data);
	        }
	        if (a3) {
	            mathx.Matrix4.multiplyTranslateMatrix(m3.data, a3.data, m3.data);
	        }
	        if (p3) {
	            p3.dirty = false;
	        }
	    ...
	    if (mesh.parent) {
	        let parentWorldMatrix = mesh.parent.worldMatrix?.data ?? mathx.Matrix4.UNIT_MATRIX4;
	...
	class Anchor3 extends Matrix4Component {
	    vec3 = new mathx.Vector3();
	    constructor(vec = mathx.Vector3.VECTOR3_ZERO) {
	        super(ANCHOR_3D, mathx.Matrix4.create(), [{
	                label: ANCHOR_3D,
	                unique: true
	            }]);
	        mathx.Vector3.fromArray(vec, 0, this.vec3);
	        this.update();
	    }
	    get x() {
	        return this.vec3[0];
	...
	class APosition3 extends Matrix4Component {
	    constructor(data = mathx.Matrix4.create()) {
	...
	class EuclidPosition3 extends APosition3 {
	    ...
	    constructor(vec3) {
	        super();
	        if (vec3) {
	            mathx.Vector3.fromArray(vec3, 0, this.vec3);
	        }
	        this.update();
	    }
	    get x() {
	...
	class EulerRotation3 extends ARotation3 {
	    ...
	    constructor(euler = {
	        ...
	        y: 0,
	        z: 0,
	        order: mathx.EulerAngle.ORDERS.XYZ,
	    }) {
	        super();
	        this.euler = euler;
	        this.update();
	    ...
	    update() {
	        mathx.Matrix4.fromEuler(this.euler, this.data);
	        this.dirty = true;
	        return this;
	    }
	}
	...
	class OrthogonalProjection extends AProjection3 {
	    ...
	    update() {
	        mathx.Matrix4.orthogonalZ0(this.options.left, this.options.right, this.options.bottom, this.options.top, this.options.near, this.options.far, this.data);
	        this.dirty = true;
	        return this;
	    }
	}
	...
	class PerspectiveProjection extends AProjection3 {
	    ...
	    update() {
	        mathx.Matrix4.perspectiveZ0(this.options.fovy, this.options.aspect, this.options.near, this.options.far, this.data);
	        this.dirty = true;
	        return this;
	    }
	}
	...
	class SphericalPosition3 extends APosition3 {
	    spherical = new mathx.Spherical();
	    #vec3 = new mathx.Vector3();
	    constructor(spherical = new Float32Array(3)) {
	        super();
	        mathx.Spherical.fromArray(spherical, this.spherical);
	        this.update();
	    }
	    get radius() {
	...
	class Vector3Scale3 extends AScale3 {
	    ...
	    update() {
	        mathx.Matrix4.fromScaling(this.vec3, this.data);
	        return this;
	    }
	}
	...
	class PerspectiveProjectionX extends AProjection3 {
	    ...
	    update() {
	        mathx.Matrix4.perspectiveZ0(this.options.fovx / this.options.aspect, this.options.aspect, this.options.near, this.options.far, this.data);
	        this.dirty = true;
	        return this;
	    }
	}

	class Renderable extends x$1.Component {
	    tags = [{
	            label: RENDERABLE,
	            unique: true
	        }];
	...
	class Texture extends x$1.Component {
	    dirty = false;
	    width;
	    height;
	...
	class Tween extends x$1.Component {
	        ...
	        if (from instanceof Float32Array && to instanceof Float32Array) {
	            if (Math.min(from.length, to.length) === 2) {
	                map.set(' ', {
	                    ...
	                    delta: mathx.Vector2.minus(to, from)
	                });
	            }
	                ...
	                map.set(' ', {
	                    type: 'vector3',
	                    origin: new Float32Array(from),
	                    delta: mathx.Vector3.minus(to, from)
	                });
	            }
	                ...
	                map.set(' ', {
	                    type: 'vector4',
	                    origin: new Float32Array(from),
	                    delta: mathx.Vector4.minus(to, from)
	                });
	            }
	            return this;
	        }
	                ...
	                if (typeof to[key] === 'number' && 'number' === typeof from[key]) {
	                    ...
	                    if (Math.min(from[key].length, to[key].length) === 2) {
	                        map.set(key, {
	                            ...
	                            delta: mathx.Vector2.minus(to[key], from[key])
	                        });
	                    }
	                        ...
	                        map.set(key, {
	                            type: 'vector3',
	                            origin: new Float32Array(from[key]),
	                            delta: mathx.Vector3.minus(to[key], from[key])
	                        });
	                    }
	                        ...
	                        map.set(key, {
	                            type: 'vector4',
	                            origin: new Float32Array(from[key]),
	                            delta: mathx.Vector4.minus(to[key], from[key])
	                        });
	                    }
	                }
	            }
	...
	var getEulerRotation3Proxy = (position) => {
	    ...
	    let euler = mathx.EulerAngle.fromMatrix4(position.data);
	    return new Proxy(position, {
	        get: (target, property) => {
	...
	let Object3$1 = class Object3 extends x$1.Entity {
	    anchor;
	    position;
	    rotation;
	    scaling;
	    ...
	    constructor(name = "Object3") {
	        ...
	        this.modelMatrix = new Matrix3Component(MODEL_2D, mathx.Matrix3.create(), [{
	                label: MODEL_3D,
	                unique: true
	            }]);
	        this.worldMatrix = new Matrix3Component(WORLD_MATRIX3, mathx.Matrix3.create(), [{
	                label: WORLD_MATRIX3,
	                unique: true
	            }]);
	    }
	...
	class Object3 extends x$1.Entity {
	    anchor;
	    position;
	    rotation;
	    scaling;
	    ...
	    constructor(name = "Object3") {
	        ...
	        this.modelMatrix = new Matrix4Component(MODEL_3D, mathx.Matrix4.create(), [{
	                label: MODEL_3D,
	                unique: true
	            }]);
	        this.worldMatrix = new Matrix4Component(WORLD_MATRIX4, mathx.Matrix4.create(), [{
	                label: WORLD_MATRIX4,
	                unique: true
	            }]);
	    }
	...
	class HashRouteSystem extends x$1.System {
	...
	class HashRouteComponent extends tree.TreeNode.mixin(x$1.Component) {
	    children = [];
...
`);
	...
	class WebGPUMesh2Renderer {
	    ...
	    render(mesh, context) {
	        ...
	        mathx.Matrix3.multiply(this.camera.projection.data, mathx.Matrix3.invert(updateModelMatrixComponent$1(this.camera).data), mvp);
	        mathx.Matrix3.multiply(mvp, mesh.worldMatrix.data, mvp);
	    ...
	    createCacheData(mesh, context) {
	        ...
	        return {
	            mvpExt: new mathx.Matrix4(),
	            mvp: new mathx.Matrix3(),
	...
	function fromMatrix3MVP(data, out = new mathx.Matrix4()) {
	    out[0] = data[0];
	    out[1] = data[1];
	    out[2] = 0;
	    out[3] = 0;
	...
	class WebGPUMesh3Renderer {
	    ...
	    render(mesh, context) {
	        ...
	        const mvp = cacheData.mvp;
	        mathx.Matrix4.multiply(this.camera.projection.data, mathx.Matrix4.invert(updateModelMatrixComponent(this.camera).data), mvp);
	        mathx.Matrix4.multiply(mvp, mesh.worldMatrix.data, mvp);
	...
	var getColorGPU = (color, result = new mathx.ColorGPU()) => {
	    if (color instanceof mathx.ColorGPU) {
	        result.set(color);
	    }
	    else if (typeof color === "string") {
	        mathx.ColorGPU.fromString(color, result);
	    }
	    else if (typeof color === "number") {
	        mathx.ColorGPU.fromHex(color, 1, result);
	    }
	    else if (color instanceof mathx.ColorRGB) {
	        mathx.ColorGPU.fromColorRGB(color, result);
	    }
	    else if (color instanceof mathx.ColorRGBA) {
	        mathx.ColorGPU.fromColorRGBA(color, result);
	    }
	    else if (color instanceof mathx.ColorHSL) {
	        mathx.ColorGPU.fromColorHSL(color.h, color.s, color.l, result);
	    }
	    else if (color instanceof Float32Array || color instanceof Array) {
	        mathx.ColorGPU.fromArray(color, result);
	    }
	    else if (color instanceof Float32Array || color instanceof Array) {
	        mathx.ColorGPU.fromArray(color, result);
	    }
	        ...
	        if ("a" in color) {
	            mathx.ColorGPU.fromJson(color, result);
	        }
	        else {
	            mathx.ColorGPU.fromJson({
	                ...color,
	                a: 1
	            }, result);
	        }
	...
	class RenderSystemInCanvas extends x$1.System {
	    context;
	    alphaMode;
	    viewport = {
	        x: 0,
	    ...
	    constructor(name, options) {
	        ...
	        this.clearColor = options.clearColor ?? new mathx.ColorGPU(0, 0, 0, 1);
	        this.autoResize = options.autoResize ?? false;
	        this.options.noDepthTexture = options.noDepthTexture ?? false;
	    }
	    clearColorGPU = new mathx.ColorGPU(0, 0, 0, 1);
	    get clearColor() {
	...
	class TweenSystem extends x$1.System {
	    query(entity) {
	    ...
	    handle(entity, time, delta) {
	        ...
	        if (from instanceof Float32Array) {
	            ...
	            if (data.type === "vector2") {
	                mathx.Vector2.multiplyScalar(data.delta, rate, from);
	                mathx.Vector2.add(data.delta, data.origin, from);
	            }
	            else if (data.type === "vector3") {
	                mathx.Vector3.multiplyScalar(data.delta, rate, from);
	                mathx.Vector3.add(data.delta, data.origin, from);
	            }
	            else if (data.type === "vector4") {
	                mathx.Vector4.multiplyScalar(data.delta, rate, from);
	                mathx.Vector4.add(data.delta, data.origin, from);
	            }
	            return this;
	        }
	        map.forEach((data, key) => {
	            if (data.type === "number") {
	                ...
	                mathx.Vector2.multiplyScalar(data.delta, rate, from[key]);
	                mathx.Vector2.add(data.delta, data.origin, from[key]);
	            }
	            else if (data.type === "vector3") {
	                mathx.Vector3.multiplyScalar(data.delta, rate, from[key]);
	                mathx.Vector3.add(data.delta, data.origin, from[key]);
	            }
	            else if (data.type === "vector4") {
	                mathx.Vector4.multiplyScalar(data.delta, rate, from[key]);
	                mathx.Vector4.add(data.delta, data.origin, from[key]);
	            }
	        });
	        return this;
	    }
	...
	Object.keys(x$1).forEach(function (k) {
		if (k !== 'default' && !exports.hasOwnProperty(k)) Object.defineProperty(exports, k, {
			enumerable: true,
			get: function () { return x$1[k]; }
		});
	});
	Object.keys(mathx).forEach(function (k) {
		if (k !== 'default' && !exports.hasOwnProperty(k)) Object.defineProperty(exports, k, {
			enumerable: true,
			get: function () { return mathx[k]; }
		});
	});
