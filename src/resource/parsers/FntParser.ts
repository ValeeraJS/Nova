import { BitmapFontJson } from "../../bitmapfont/BitmapFontType";
import { Texture } from "../../systems/render/texture/Texture";

export const FntParser = async (data: string, ...blobs: Blob[]) => {
	if (!data) {
		throw new Error('no data provided');
	}
	data = data.trim();

	const output: BitmapFontJson = {
		pages: [],
		chars: [],
		kernings: [],
	};
	...
	for (let i = 0; i < lines.length; i++) {
		const lineData: any = splitLine(lines[i], i);
